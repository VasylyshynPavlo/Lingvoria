...
public class WordService(LingvoriaDbContext context, IMapper mapper) : IWordService
{
    ...
    {
        var collection = await context.WordsCollections.Find(c => c.Id.ToString() == collectionId).FirstOrDefaultAsync();
    ...
    public async Task<Response> GetCollections(string userId)
    {
        ...
        return collectionToReturn.Count == 0 ? new Response(404, "You don't have any collections") : new Response(200, "Collections Retrieved", collectionToReturn);
    }
    ...
    public async Task<Response> GetCollectionById(string collectionId)
    {
        var collection = await context.WordsCollections.Find(w => w.Id == new ObjectId(collectionId)).FirstOrDefaultAsync();
        if (collection == null) return new Response(404, "Collection Not Found");
    ...
    public async Task<Response> AddExample(CreateExampleFrom example)
    {
    ...
    public async Task<Response> UpdateExample(string collectionId, string wordId, string exampleId, string? text, string? translate)
    {
        var collection = await context.WordsCollections.Find(w => w.Id == new ObjectId(collectionId)).FirstOrDefaultAsync();
        if (collection == null) return new Response(404, "Collection Not Found");
        var word = collection.Words.FirstOrDefault(w => w.Id == new ObjectId(wordId));
